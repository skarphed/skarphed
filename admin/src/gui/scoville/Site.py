...
class WidgetContainer(gtk.HBox):
    def __init__(self, par, site, spaceId, widget=None):
        ...
        self.view.set_name("siteWidget_"+str(self.siteId)+"_"+str(self.spaceId))
    ...
    def render(self):
        ...
        try:
            widget = site.getWidgetInSpace(self.spaceId)
        ...
        if len(self.store) > 0:
            if widget is None:
                ...
                self.store[0][1] = widget.getName()
                self.store[0][2] = widget.getLocalId()
        else:
            self.col_wdg.set_title("Space "+str(self.spaceId)+": Free")
            if widget is not None:
                self.store.append((IconStock.WIDGET, widget.getName(), widget.getLocalId()))
    ...
    def dragReceiveWidget(self, view, context, x, y, selection, info, timestamp):
        ...
        if sourceName  == "siteWidgetList"+str(site.getLocalId()):
            validDrop = True
        if sourceName.startswith("siteWidget_"+str(site.getLocalId())) and sourceName[-1:] != str(self.spaceId):
            validDrop = True
            move = True
        if not validDrop:
            return
        ...
        if move:
            site.removeWidgetFromSpace(sourceName[-1:])
        widget = self.getApplication().getLocalObjectById(int(selection.data))
        site.addWidgetToSpace(self.spaceId, widget)
...
class SitePage(GenericObjectPage):
    def __init__(self, par, site):
        ...
        self.wdg = PageFrame(self,"Widgets", IconStock.WIDGET)
        ...
        for i in range(site.getSpaceCount()):
            widget = site.getWidgetInSpace(i+1)
            if widget is None:
                self.wdg_spacewidgets.append(WidgetContainer(self,site,i+1))
            else:
                self.wdg_spacewidgets.append(WidgetContainer(self,site,i+1,widget))
    ...
    def render(self):
        ...
        self.info_minimap.set_from_file(site.getMinimap())
        
        usedWidgetIds = site.getUsedWidgetIds()
        self.wdg_store.clear()
        widgets = site.getScoville().modules.getAllWidgets()
