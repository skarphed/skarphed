...
class Repository(object):
    ...
    def upload_module(self, environ, data, signature):
        ...
        js_mandatory = 0
        if manifest.has_key('js_mandatory'):
            if manifest['js_mandatory'] == 'mandatory':
                js_mandatory = 2
            elif manifest['js_mandatory'] == 'supported':
                js_mandatory = 1
            elif manifest['js_mandatory'] == 'no':
                js_mandatory = 0
            else:
                raise create_repository_exception(RepositoryErrorCode.UPLOAD_JS_MANDATORY)
